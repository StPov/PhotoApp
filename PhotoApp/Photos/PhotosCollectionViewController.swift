...
class PhotosCollectionViewController: UICollectionViewController {
    ...
    private func setupNavigationBar() {
        let titleLabel = UILabel(text: "Фотограмм", font: .systemFont(ofSize: 15, weight: .medium), textColor: #colorLiteral(red: 0.5019607843, green: 0.4980392157, blue: 0.4980392157, alpha: 1))
    ...
    override func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
            return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { suggestedActions in
                let save = UIAction(title: "Save", image: UIImage(named: "save")) { action in
                    ...
                    let navVC = tabbar.viewControllers?[1] as! UINavigationController
                    let likesVC = navVC.topViewController as! LikesCollectionViewController
                    likesVC.photos.append(self.photos[indexPath.item])
                    
                    likesVC.collectionView.reloadData()
                            
                    self.refresh()
                }
                let open = UIAction(title: "Open", image: UIImage(named: "open")) { action in
                }
                return UIMenu(title: "", children: [save, open])
            }
        }
}
